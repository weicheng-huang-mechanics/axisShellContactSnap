...
void display(void)
{
	while ( myWorld.simulationRunning() > 0)
	{
		...
		glBegin(GL_LINES);
		Vector2d xCenter = myWorld.getScaledPos(myWorld.getNv() - 1);
		double radius = myWorld.getBallRadius();
		double deltaT = 2 * M_PI / 100;

		for (int i = 0; i < 100; i++)
		{
			Vector2d xCurrent_1;
			xCurrent_1(0) = xCenter(0) + radius * cos(i * deltaT);
			xCurrent_1(1) = xCenter(1) + radius * sin(i * deltaT);

			Vector2d xCurrent_2;
			xCurrent_2(0) = xCenter(0) + radius * cos( (i+1) * deltaT);
			xCurrent_2(1) = xCenter(1) + radius * sin( (i+1) * deltaT);
		...
		glPointSize(2.0);
		glBegin(GL_POINTS);
		for (int i = 0; i < myWorld.getVectex(); i++)
		{
			if (myWorld.getIfContact(i) == 1)
			{
				Vector2d xCurrent_1 = myWorld.getScaledPos(i);
				glVertex3f(-xCurrent_1(0), xCurrent_1(1)-0.3, 0.0);
				glVertex3f(xCurrent_1(0), xCurrent_1(1)-0.3, 0.0);
			}
		}
